    ...
    <div className="flex h-screen w-[420px] flex-col bg-zinc-950 text-zinc-100">
      ...
      {collections.length > 0 && (
        <div className="border-b border-white/10 px-3 py-2">
          <p className="mb-1 px-1 text-[11px] font-medium uppercase tracking-wide text-zinc-500">
            Collections
          </p>
          <div className="flex flex-wrap gap-1">
            {collections.map((c) => (
              <button
                key={c.id}
                type="button"
                className="inline-flex items-center gap-1 rounded-lg bg-zinc-900 px-2 py-1 text-xs text-zinc-300 hover:bg-zinc-800"
              >
                <span
      ...
      <div className="flex items-center justify-between border-t border-white/10 px-4 py-2 text-xs text-zinc-500">
        <div className="flex items-center gap-3">
          ...
          <button type="button" className="inline-flex items-center gap-1 hover:text-zinc-300">
...
export function TrayShell({ children }: { children: React.ReactNode }) {
  return (
    <div className={cn("h-full w-full", "dark")}>{children}</div>
  );
}
